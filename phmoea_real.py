...
class PHMOEAReal:
    ...
    def _compute_scores(self, F: np.ndarray, rank: np.ndarray, crowd: np.ndarray, t: int) -> np.ndarray:
        """Compute stage-dependent composite score R_i."""
        # normalize crowding within generation
        cmin, cmax = float(np.min(crowd)), float(np.max(crowd))
        chat = (crowd - cmin) / (cmax - cmin + 1e-12)
        s = (1.0 / (1.0 + rank.astype(float))) + self.p.lam * chat
        # normalize objectives to [0,1] within generation
        f1 = F[:, 0]
        f2 = F[:, 1]
        f1hat = (f1 - float(np.min(f1))) / (float(np.max(f1) - np.min(f1)) + 1e-12)
        f2hat = (f2 - float(np.min(f2))) / (float(np.max(f2) - np.min(f2)) + 1e-12)
        g = self.p.w * f1hat + (1.0 - self.p.w) * f2hat
        phi = t / max(1, self.p.max_gens)
        if phi < self.p.kappa1:
            R = s
        elif phi < self.p.kappa2:
            # linear blend
            alpha = (phi - self.p.kappa1) / (self.p.kappa2 - self.p.kappa1 + 1e-12)
            R = alpha * s + (1.0 - alpha) * g
        else:
            R = g + self.p.gamma * chat
        return R
    ...
    def _update_player_archives(self, pop: List[Individual], fronts: List[List[int]], scores: np.ndarray) -> None:
        # use the current non-dominated front for heat updates
        if not fronts:
            return
        nd = fronts[0]
        for idx in nd:
            bins = gene_to_player_bins(pop[idx].gene, K_cont=self.p.K_cont_bins)
            contrib = float(1.0 / (1.0 + scores[idx]))
            for j, b in enumerate(bins):
                self.heat[j][b] = self.heat[j].get(b, 0.0) + contrib
    ...
    def _make_offspring(self, pop: List[Individual], scores: np.ndarray, t: int) -> List[Individual]:
        ...
        # elites by lowest scores
        order = np.argsort(scores)
        elites = [pop[int(i)] for i in order[:n_par]]
        off: List[Individual] = [Individual(gene=e.gene.copy(), F=e.F.copy(), canon=e.canon, x_values=e.x_values) for e in elites]
        ...
        def gen_child(use_hot_pool: bool) -> Individual:
            ...
            # pool-biased replacement per-dim
            if use_hot_pool:
                bias_gene = self._assemble_from_pool(hot_bins)
            else:
                # non-hot with cold compensation
                bins = []
                for j in range(24):
