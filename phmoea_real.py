...
class PHMOEAReal:
    ...
    def _compute_scores(self, F: np.ndarray, rank: np.ndarray, crowd: np.ndarray, t: int) -> np.ndarray:
        """Compute the stage-dependent archive-update reward R_i.

        This version is consistent with Eq. (19) in the paper:
        - all components are larger-is-better scores;
        - s_i is the rank--diversity score;
        - g_i is the objective-guided utility converted from minimized objectives;
        - R_i is later normalized into archive-update weights omega_i.
        """
        eps = 1e-12

        # Normalize crowding distance to [0, 1].
        # NSGA-II assigns infinite crowding distance to boundary solutions;
        # replace inf by the largest finite value before min-max normalization.
        ...
        # Rank--diversity score: better rank and larger crowding distance
        # produce a larger score.
        s = (1.0 / (1.0 + rank.astype(float))) + self.p.lam * chat

        # Objective-guided utility: both objectives are minimized, so the
        # weighted normalized objective value is converted by 1 - cost.
        f1 = F[:, 0].astype(float)
        f2 = F[:, 1].astype(float)
        ...
        if phi < self.p.kappa1:
            ...
            # Convex combination avoids an unnecessary scale increase.
    ...
    def _update_player_archives(self, pop: List[Individual], fronts: List[List[int]], scores: np.ndarray) -> None:
        ...
        The paper normalizes R_i over the current population to obtain
        archive-update weights omega_i. All individuals in the current
        population contribute to the heat archive, and higher-reward
        individuals contribute larger weights.
        """
        if len(pop) == 0:
            return
